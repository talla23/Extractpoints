...
(function (codec) {
    var PulseV40x10Parser = (function () {
        ...
        PulseV40x10Parser.prototype.parseFrame = function (payload, configuration, network) {
            ...
            appContent['samplingPeriod'] = { 'unit': 's', 'value': readUInt16BE(payload, 6) * 2 };
            appContent['calculatedSendingPeriod'] = { 'unit': 's',
                'value': readUInt16BE(payload, 3) * readUInt16BE(payload, 6) * 2 };
            if (payload[2] === 2) {
                appContent['flowCalculationPeriod'] = { 'unit': 's', 'value': readUInt16BE(payload, 9) * 20 };
            }
            else {
                appContent['flowCalculationPeriod'] = { 'unit': 'm', 'value': readUInt16BE(payload, 9) };
            }
            if (knownNetwork === 'lora868') {
                appContent['redundantSamples'] = readUInt8(payload, 27);
            }
            ...
            if (knownNetwork === 'lora868') {
                chA['leakageDetection'] = {
                    'overflowAlarmTriggerThreshold': readUInt16BE(payload, 11),
                    'threshold': readUInt16BE(payload, 15),
                    'dailyPeriodsBelowWhichLeakageAlarmTriggered': readUInt16BE(payload, 19)
                };
                chA['tamper'] = {
                    'activated': Boolean(payload[5] & 0x08),
                    'samplePeriodForDetection': { 'unit': 's', 'value': readUInt8(payload, 23) * 10 },
                    'threshold': readUInt8(payload, 24)
                };
            }
            ...
            if (knownNetwork === 'lora868') {
                chB['leakageDetection'] = {
                    'overflowAlarmTriggerThreshold': readUInt16BE(payload, 13),
                    'threshold': readUInt16BE(payload, 17),
                    'dailyPeriodsBelowWhichLeakageAlarmTriggered': readUInt16BE(payload, 21)
                };
                chB['tamper'] = {
                    'activated': Boolean(payload[5] & 0x80),
                    'samplePeriodForDetection': { 'unit': 's', 'value': readUInt8(payload, 25) * 10 },
                    'threshold': readUInt8(payload, 26)
                };
            }
            else if (knownNetwork === 'sigfox') {
...
(function (codec) {
    var PulseV40x11Parser = (function () {
        ...
        PulseV40x11Parser.prototype.parseFrame = function (payload, configuration, network) {
            ...
            var chB = { 'name': 'channel B' };
            chA['leakageDetection'] = {
                'overflowAlarmTriggerThreshold': readUInt16BE(payload, 2),
                'threshold': readUInt16BE(payload, 6)
            };
            chB['leakageDetection'] = {
                'overflowAlarmTriggerThreshold': readUInt16BE(payload, 4),
                'threshold': readUInt16BE(payload, 8)
            };
            appContent['channels'] = [chA, chB];
            return appContent;
        };
...
(function (codec) {
    var PulseV40x12Parser = (function () {
        ...
        PulseV40x12Parser.prototype.parseFrame = function (payload, configuration, network) {
            ...
            var chB = { 'name': 'channel B' };
            chA['leakageDetection'] = {
                'dailyPeriodsBelowWhichLeakageAlarmTriggered': readUInt16BE(payload, 2),
            };
            chA['tamper'] = {
                'samplePeriodForDetection': { 'unit': 's', 'value': readUInt8(payload, 6) * 10 },
                'threshold': readUInt8(payload, 7)
            };
            chB['leakageDetection'] = {
                'dailyPeriodsBelowWhichLeakageAlarmTriggered': readUInt16BE(payload, 4),
            };
            chB['tamper'] = {
                'samplePeriodForDetection': { 'unit': 's', 'value': readUInt8(payload, 8) * 10 },
                'threshold': readUInt8(payload, 9)
            };
            appContent['channels'] = [chA, chB];
            return appContent;
        };
...
(function (codec) {
    var PulseV40x30Parser = (function () {
        ...
        PulseV40x30Parser.prototype.parseFrame = function (payload, configuration, network) {
            ...
            if (payload[1] & 0x04) {
                var myDate = new Date((readUInt32BE(payload, 11) + 1356998400) * 1000);
            ...
            chA['flow'] = {
                ...
                'last24hMin': readUInt16BE(payload, 7),
                'last24hMax': readUInt16BE(payload, 3)
            };
            chA['tamperAlarm'] = Boolean(payload[2] & 0x04);
            chA['leakageAlarm'] = Boolean(payload[2] & 0x10);
            chB['flow'] = {
                'alarm': Boolean(payload[2] & 0x02),
                'last24hMin': readUInt16BE(payload, 9),
                'last24hMax': readUInt16BE(payload, 5)
            };
...
(function (codec) {
    var PulseV40x46Parser = (function () {
        ...
        PulseV40x46Parser.prototype.parseFrame = function (payload, configuration, network) {
            ...
            if (payload[1] & 0x04) {
                var myDate = new Date((readUInt32BE(payload, 10) + 1356998400) * 1000);
                appContent['timestamp'] = myDate.toJSON().replace('Z', '');
            }
            appContent['decodingInfo'] = 'counterValues: [Channel A, Channel B]';
            appContent['counterValues'] = [readUInt32BE(payload, 2), readUInt32BE(payload, 6)];
            return appContent;
        };
...
(function (codec) {
    var PulseV40x47Parser = (function () {
        ...
        PulseV40x47Parser.prototype.parseFrame = function (payload, configuration, network) {
            ...
            if (payload[1] & 0x04) {
                var myDate = new Date((readUInt32BE(payload, 6) + 1356998400) * 1000);
                appContent['timestamp'] = myDate.toJSON().replace('Z', '');
            }
            appContent['decodingInfo'] = 'flowValues: [Channel A, Channel B]';
            appContent['flowValues'] = [readUInt16BE(payload, 2), readUInt16BE(payload, 4)];
            return appContent;
        };
...
(function (codec) {
    var PulseV40x5aParser = (function () {
        ...
        PulseV40x5aParser.prototype.parseFrame = function (payload, configuration, network) {
            var absCounterValue = readUInt32BE(payload, 2);
            ...
            if (payload[1] & 0x04) {
                var myDate = new Date((readUInt32BE(payload, payload.length - 4) + 1356998400) * 1000);
                appContent['timestamp'] = myDate.toJSON().replace('Z', '');
            }
            for (var offset = 6; offset < payloadLength; offset += 2) {
                absCounterValue -= readUInt16BE(payload, offset);
        ...
        PulseV40x5aParser.prototype.getReadingFrequency = function (configuration) {
            return configuration.byteLength > 0 ? readUInt16BE(configuration, 8) * readUInt16BE(configuration, 6) * 2 : null;
        };
