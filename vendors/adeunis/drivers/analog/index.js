    ...
    var Analog0x11Parser = (function () {
        ...
        Analog0x11Parser.prototype.parseFrame = function (payload, configuration, network) {
            ...
            appContent['threshold'] = {
                ...
                'unit': '\u00B5' + 'V or 10 nA',
                'high': {
                    'value': readUInt32BE(payload, 1) & 0x00ffffff,
                    'hysteresis': readUInt32BE(payload, 4) & 0x00ffffff,
                }
            };
            return appContent;
        };
...
(function (codec) {
    var Analog0x42Parser = (function () {
        ...
        Analog0x42Parser.prototype.parseFrame = function (payload, configuration, network) {
            ...
            var rawValue = readUInt32BE(payload, 2) & 0x00ffffff;
            if (type === 1) {
                ch1['unit'] = 'V';
            ...
            type = payload[6] & 0x0f;
            rawValue = readUInt32BE(payload, 6) & 0x00ffffff;
            if (type === 1) {
                ch2['unit'] = 'V';
