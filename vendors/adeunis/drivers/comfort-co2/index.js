"use strict";
//Prototype method readInt8 takes the object prototype's value at index offset. 
//If that value has a value of 0b 1xxx xxxx, then it returns this value, minus 0b 0001 0000 0000
...
//This prototype method readUInt8 returns the value that is at the obejct's index offset.
function readUInt8(payload, offset) {
    var buffer = payload;
    return buffer[offset];
};

//This prototype method readInt16BE shifts 8 times left the value of the object prototype at index offset,
//Operates a logical OR with the value at index offset + 1.
//Then it returns this value minus 0b 0001 0000 0000 0000 if it is 0b 0001 0000 0000 0000
//Else it returns this value.
...
//This method returns shifts 8 times the value at index offset, performs a logical OR with the value at index offset + 1 and returns its value.
...
(function (codec) {
    var ComfortCo20x10Parser = (function () {
        ...
        ComfortCo20x10Parser.prototype.parseFrame = function (payload, configuration, network) {
            ...
            appContent['dailyFrameActivated'] = (Boolean)((readUInt16BE(payload, 2) === 0) ? false : true),
            ...
            if (payload.length >= 12) {
                appContent['blackOutDuration'] = { 'unit': 'h', 'value': readUInt8(payload, 11) };
                appContent['blackOutStartTime'] = { 'unit': 'h', 'value': readUInt8(payload, 12) };
            }
            return appContent;
        };
...
(function (codec) {
    var ComfortCo20x30Parser = (function () {
        ...
        ComfortCo20x30Parser.prototype.parseFrame = function (payload, configuration, network) {
            ...
            if (payload[1] & 0x04) {
                var myDate = new Date((readUInt32BE(payload, 19) + 1356998400) * 1000);
                appContent['timestamp'] = myDate.toJSON().replace('Z', '');
            }
            appContent['icone'] = readUInt8(payload, 2);
            appContent['qaiRedDuration'] = { 'unit': 'min', 'value': (readUInt8(payload, 3) * 10) };
            appContent['temperatureMax'] = { 'unit': '\u00B0' + 'C', 'value': readInt16BE(payload, 4) / 10 };
            appContent['co2Max'] = { 'unit': 'ppm', 'value': readUInt16BE(payload, 6) };
            appContent['humidityMax'] = { 'unit': '%', 'value': readUInt8(payload, 8) };
            appContent['temperatureMin'] = { 'unit': '\u00B0' + 'C', 'value': readInt16BE(payload, 9) / 10 };
            switch (payload.length) {
                case 11:
                    break;
                case 19:
                case 23:
                    appContent['co2Min'] = { 'unit': 'ppm', 'value': readUInt16BE(payload, 11) };
                    appContent['humidityMin'] = { 'unit': '%', 'value': readUInt8(payload, 13) };
                    appContent['temperatureAverage'] = { 'unit': '\u00B0' + 'C', 'value': readInt16BE(payload, 14) / 10 };
                    appContent['co2Average'] = { 'unit': 'ppm', 'value': readUInt16BE(payload, 16) };
                    appContent['humidityAverage'] = { 'unit': '%', 'value': readUInt8(payload, 18) };
                    break;
                default:
                    appContent.partialDecoding = codec.PartialDecodingReason.MISSING_NETWORK;
                    break;
...
(function (codec) {
    var ComfortCo20x6aParser = (function () {
        ...
        ComfortCo20x6aParser.prototype.parseFrame = function (payload, configuration, network) {
            ...
            if (payload[1] & 0x04) {
                var myDate = new Date((readUInt32BE(payload, payloadLength) + 1356998400) * 1000);
                appContent['timestamp'] = myDate.toJSON().replace('Z', '');
            }
            var rawValue;
            ...
            for (var offset = 2; offset < payloadLength; offset += 5) {
                ...
                rawValue = readUInt16BE(payload, offset + 3);
                CO2.push(rawValue);
            }
    ...
    var ComfortCo20x6bParser = (function () {
        ...
        ComfortCo20x6bParser.prototype.parseFrame = function (payload, configuration, network) {
            ...
            appContent['alarmCo2'] = {
                ...
                'co2': { 'unit': 'ppm', 'value': readInt16BE(payload, 6) }
            };
            return appContent;
        };
