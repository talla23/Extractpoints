    ...
    var TempV40x30Parser = (function () {
        ...
        TempV40x30Parser.prototype.parseFrame = function (payload, configuration, network) {
            ...
            if (payload[1] & 0x04) {
                ...
                var myDate = new Date((readUInt32BE(payload, offsetType) + 1356998400) * 1000);
                appContent['timestamp'] = myDate.toJSON().replace('Z', '');
            }
            var temperatures = [];
            temperatures.push({ 'name': 'temperature 1', 'unit': '\u00B0' + 'C', 'value': readInt16BE(payload, 2) / 10 });
            if (nbSensors === 2) {
                temperatures.push({ 'name': 'temperature 2', 'unit': '\u00B0' + 'C', 'value': readInt16BE(payload, 4) / 10 });
            }
    ...
    var TempV40x57Parser = (function () {
        ...
        TempV40x57Parser.prototype.parseFrame = function (payload, configuration, network) {
            ...
            for (var offset = 2; offset < payloadLength; offset += 2 * nbSensors) {
                ...
                if (nbSensors === 2) {
                    rawValue = readInt16BE(payload, offset + 2);
...
(function (codec) {
    var TempV40x58Parser = (function () {
        ...
        TempV40x58Parser.prototype.parseFrame = function (payload, configuration, network) {
            ...
            alarms.push({
                'name': 'temperature 1',
                'alarmStatus': this.getAlarmStatusText(readUInt8(payload, 2)),
                'temperature': { 'unit': '\u00B0' + 'C', 'value': readInt16BE(payload, 3) / 10 },
            });
            if (nbSensors === 2) {
                alarms.push({
                    'name': 'temperature 2',
                    'alarmStatus': this.getAlarmStatusText(readUInt8(payload, 5)),
                    'temperature': { 'unit': '\u00B0' + 'C', 'value': readInt16BE(payload, 6) / 10 }
                });
            }
            if (payload[1] & 0x04) {
                var myDate = new Date((readUInt32BE(payload, offsetType) + 1356998400) * 1000);
