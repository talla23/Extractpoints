...
(function (codec) {
    var Drycontacts20x10Parser = (function () {
        ...
        Drycontacts20x10Parser.prototype.parseFrame = function (payload, configuration, network) {
            ...
            appContent['keepAlivePeriod'] = { 'unit': 's', 'value': readUInt16BE(payload, 2) * 10 };
            appContent['transmitPeriod'] = { 'unit': 's', 'value': readUInt16BE(payload, 4) * 10 };
...
(function (codec) {
    var Drycontacts20x30Parser = (function () {
        ...
        Drycontacts20x30Parser.prototype.parseFrame = function (payload, configuration, network) {
            ...
            appContent['channelA'] = { 'value': readUInt16BE(payload, 2), 'state': Boolean(payload[10] & 0x01) };
            appContent['channelB'] = { 'value': readUInt16BE(payload, 4), 'state': Boolean(payload[10] & 0x02) };
            appContent['channelC'] = { 'value': readUInt16BE(payload, 6), 'state': Boolean(payload[10] & 0x04) };
            appContent['channelD'] = { 'value': readUInt16BE(payload, 8), 'state': Boolean(payload[10] & 0x08) };
            return appContent;
        };
...
(function (codec) {
    var Drycontacts20x40Parser = (function () {
        ...
        Drycontacts20x40Parser.prototype.parseFrame = function (payload, configuration, network) {
            ...
            appContent['channelA'] = {
                'value': readUInt16BE(payload, 2), 'currentState': Boolean(payload[10] & 0x01),
                'previousFrameState': Boolean(payload[10] & 0x02)
            };
            appContent['channelB'] = {
                'value': readUInt16BE(payload, 4), 'currentState': Boolean(payload[10] & 0x04),
                'previousFrameState': Boolean(payload[10] & 0x08)
            };
            appContent['channelC'] = {
                'value': readUInt16BE(payload, 6), 'currentState': Boolean(payload[10] & 0x10),
                'previousFrameState': Boolean(payload[10] & 0x20)
            };
            appContent['channelD'] = {
                'value': readUInt16BE(payload, 8), 'currentState': Boolean(payload[10] & 0x40),
...
(function (codec) {
    var Drycontacts20x41Parser = (function () {
        ...
        Drycontacts20x41Parser.prototype.parseFrame = function (payload, configuration, network) {
            ...
            if (payload[1] & 0x04) {
                var myDate = new Date((readUInt32BE(payload, 5) + 1356998400) * 1000);
                appContent['timestamp'] = myDate.toJSON().replace('Z', '');
            }
            appContent['channel'] = readUInt8(payload, 2);
            appContent['durationThreshold'] = { 'unit': 'min', 'value': readUInt16BE(payload, 3) };
            return appContent;
        };
...
(function (codec) {
    var Drycontacts20x59Parser = (function () {
        ...
        Drycontacts20x59Parser.prototype.parseFrame = function (payload, configuration, network) {
            ...
            var offset = 3;
            if (payload[2] & 0x01) {
                appContent['channelATimeCounter'] = { 'unit': 's', 'value': readUInt32BE(payload, offset) };
                offset += 4;
            }
            if (payload[2] & 0x02) {
                appContent['channelBTimeCounter'] = { 'unit': 's', 'value': readUInt32BE(payload, offset) };
                offset += 4;
            }
            if (payload[2] & 0x04) {
                appContent['channelCTimeCounter'] = { 'unit': 's', 'value': readUInt32BE(payload, offset) };
                offset += 4;
            }
            if (payload[2] & 0x08) {
                appContent['channelDTimeCounter'] = { 'unit': 's', 'value': readUInt32BE(payload, offset) };
            }
            return appContent;
        };
