...
function Decoder(bytes) {
  ...
  // (array) of bytes to an object of fields.
	...
	if (bytes.length >= 1) 
	{
		...
		const MsgType = bytes[0]; // Type of message
		
		if (MsgType == 0x11) //Alarms
		{
			index = 1;
			const NbSecond = (bytes[index++]*256*256*256+bytes[index++]*256*256+bytes[index++]*256+bytes[index++]);
			const LogicCombi_Alarm = bytes[index++];
			const Analogic_Alarm = bytes[index++];
			const System_Alarm = bytes[index++];
			const Protection_Alarm = bytes[index++];
		...
		if (MsgType == 2) //periodic data
		{
			...
			let NbSecond;
			let PulseMeter;
			const Profile = ((bytes[1] & 0xF0) >>> 4);
			const Profile_version = (bytes[1] & 0x0F);
			...
			if (Profile === 1) // Single-load – Energies (consumption/production)
			{
				...
				const Ea_plus = (readBytes(bytes, 6, 8));
				const Ea_moins = (readBytes(bytes, 14, 8));
				const Er_plus = (readBytes(bytes, 22, 8));
				const Er_moins = (readBytes(bytes, 30, 8));
				const PulseMeter = (readBytes(bytes, 38, 8));
			...
			{
				NbSecond = (readBytes(bytes, 2 , 4));
				const Ea_plus_Load1 = (readBytes(bytes, 6, 4));
				const Er_plus_Load1 = (readBytes(bytes, 10, 4));
				const Ea_plus_Load2 = (readBytes(bytes, 14, 4));
				const Er_plus_Load2 = (readBytes(bytes, 18, 4));
				const Ea_plus_Load3 = (readBytes(bytes, 22, 4));
				const Er_plus_Load3 = (readBytes(bytes, 16, 4));
				const Ea_plus_Load4 = (readBytes(bytes, 30, 4));
				const Er_plus_Load4 = (readBytes(bytes, 34, 4));
			...
			if (Profile === 3) //Profile 3: Multi-load – Energies (consumption/production)
			{
				NbSecond = (readBytes(bytes, 2 , 4));
				const Ea_plus_Load1 = (readBytes(bytes, 6, 4));
				const Ea_moins_Load1 = (readBytes(bytes, 10, 4));
				const Ea_plus_Load2 = (readBytes(bytes, 14, 4));
				const Ea_moins_Load2 = (readBytes(bytes, 18, 4));
				const Ea_plus_Load3 = (readBytes(bytes, 22, 4));
				const Ea_moins_Load3 = (readBytes(bytes, 16, 4));
				const Ea_plus_Load4 = (readBytes(bytes, 30, 4));
				const Ea_moins_Load4 = (readBytes(bytes, 34, 4));
				PulseMeter = (readBytes(bytes, 38, 8));
			...
			{
				 NbSecond = (readBytes(bytes, 2 , 4));
				const Pmoy = U32toS32(readBytes(bytes, 6, 4))/1000;
				const Qmoy = U32toS32(readBytes(bytes, 10, 4))/1000;
				const Smoy = (readBytes(bytes, 14, 4))/1000;
				const Pf_Moy = U32toS16(readBytes(bytes, 18, 2));
				const Pf_Type = (readBytes(bytes, 20, 2));
				const I1_Moy = (readBytes(bytes, 22, 4))/1000;
				const I2_Moy = (readBytes(bytes, 26, 4))/1000;
				const I3_Moy = (readBytes(bytes, 30, 4))/1000;
				const F_moy = (readBytes(bytes, 34, 4))/1000;
				DIaVM = (readBytes(bytes, 38, 2)); Decode_DIaVM = true;
				const Temp_1 = U32toS16(readBytes(bytes, 40, 2));
				const Temp_2 = U32toS16(readBytes(bytes, 42, 2));
				const Temp_3 = U32toS16(readBytes(bytes, 44, 2));
				const CounterStatus2 = (readBytes(bytes, 46, 2));
				CounterStatus = (readBytes(bytes, 48, 2)); Decode_CounterStatus = true;
			...
			{
				NbSecond = (readBytes(bytes, 2 , 4));
				const Pmoy_Load1 = U32toS32(readBytes(bytes, 6, 4))/1000;
				const Qmoy_Load1 = U32toS32(readBytes(bytes, 10, 4))/1000;
				const Pmoy_Load2 = U32toS32(readBytes(bytes, 14, 4))/1000;
				const Qmoy_Load2 = U32toS32(readBytes(bytes, 18, 4))/1000;
				const Pmoy_Load3 = U32toS32(readBytes(bytes, 22, 4))/1000;
				const Qmoy_Load3 = U32toS32(readBytes(bytes, 26, 4))/1000;
				const Pmoy_Load4 = U32toS32(readBytes(bytes, 30, 4))/1000;
				const Qmoy_Load4 = U32toS32(readBytes(bytes, 34, 4))/1000;
			...
			{
				const Date_t0 = (readBytes(bytes, 2 , 4));
				const P_Plus_t0 = (readBytes(bytes, 6, 4))/1000;
				const P_Moins_t0 = (readBytes(bytes, 10, 4))/1000;
				const Q_Plus_t0 = (readBytes(bytes, 14, 4))/1000;
				const Q_Moins_t0 = (readBytes(bytes, 18, 4))/1000;
				const type_t0 = (readBytes(bytes, 22, 2));

				const Date_t1 = (readBytes(bytes, 24 , 4));
				const P_Plus_t1 = (readBytes(bytes, 28, 4))/1000;
				const P_Moins_t1 = (readBytes(bytes, 32, 4))/1000;
				const Q_Plus_t1 = (readBytes(bytes, 36, 4))/1000;
				const Q_Moins_t1 = (readBytes(bytes, 40, 4))/1000;
				const type_t1 = (readBytes(bytes, 44, 2));
			...
			{
				const Date_t0 = (readBytes(bytes, 2 , 4));
				const P_Plus_t0_load1 = (readBytes(bytes, 6, 4))/1000;
				const P_Plus_t0_load2 = (readBytes(bytes, 10, 4))/1000;
				const P_Plus_t0_load3 = (readBytes(bytes, 14, 4))/1000;
				const P_Plus_t0_load4 = (readBytes(bytes, 18, 4))/1000;
				const type_t0 = (readBytes(bytes, 22, 2));

				const Date_t1 = (readBytes(bytes, 24 , 4));
				const P_Plus_t1_load1 = (readBytes(bytes, 28, 4))/1000;
				const P_Plus_t1_load2 = (readBytes(bytes, 32, 4))/1000;
				const P_Plus_t1_load3 = (readBytes(bytes, 36, 4))/1000;
				const P_Plus_t1_load4 = (readBytes(bytes, 40, 4))/1000;
				const type_t1 = (readBytes(bytes, 44, 2));
