...
function getInputs(examples) {
    ...
    let inputs = examples.filter(example => example.type === "uplink").map(obj => {
        let input = {};
        let description = obj.description ?? "No description"; // Use description by default if not present
        if (obj.output) {
            input = obj.output.data ?? obj.output;
        } else if (obj.data) {
            input = obj.data;
        }
        return { message: input, description: description }; // put description in the return object
    });
