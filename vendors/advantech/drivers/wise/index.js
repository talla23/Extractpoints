...
//If program executes in NodeRed, get input data from msg.payload.data
    ...
    if (receivedString === undefined) {
        throw "invalid payload: no data received"
    }
    ...
    var i, arrLength;
    var hexArr = []; //translated hex array from input string
//var payload = {};
    var hexPayloadArr = [];
...
////////////////////////////////////////////
    ...
    function parseAxisData(index, bIsSensorEventExist, extMask, jsonObj) {
        ...
        if (extMask & MASK_PAYLOAD_SENSOR_EXTMASK_DISPLACEMENT) {
            ...
            jsonObj['PeakToPeak Displacement'] = jsonObj['Peak-to-Peak Displacement'];
        }
    ...
    function coilParse(index) {
        ...
        if (mask & MASK_PAYLOAD_COIL_STATUS) {
            ...
            message['RtuCoil' + port + channelIndex].Status = message['RtuCoil' + port + '-' + channelIndex].Status;
            if (version > 0) length -= 1;
        }
        if (mask & MASK_PAYLOAD_COIL_VALUE) {
            message['RtuCoil' + port + '-' + channelIndex].Data = hexArr[index++];
            message['RtuCoil' + port + channelIndex].Data = message['RtuCoil' + port + '-' + channelIndex].Data;
            if (version > 0) length -= 1;
        }
    ...
    function registerParse(index) {
        ...
        if (mask & MASK_PAYLOAD_REGISTER_STATUS) {
            ...
            message['RtuRegister' + port + channelIndex].Status = message['RtuRegister' + port + '-' + channelIndex].Status;
            if (version > 0) length -= 1;
        }
        if (mask & MASK_PAYLOAD_REGISTER_VALUE) {
            message['RtuRegister' + port + '-' + channelIndex].Data = translateInt16(hexArr[index++], hexArr[index++]);
            message['RtuRegister' + port + channelIndex].Data = message['RtuRegister' + port + '-' + channelIndex].Data;
            if (version > 0) length -= 2;
        }
...
////////////////////////////////////////////
    ...
    try {
        if (arrLength < MIN_FRAME_LENGTH || arrLength % 2 !== 0) {
            throw "invalid payload: received frame length error"
        }
        ...
        for (i = 0; i < arrLength; i++) {
            hexArr.push(parseInt(receivedString.substr(i * 2, 2), 16)); //parse hex
        }
        ...
        if (!(hexArr[0] & MASK_HEADER_FIRST_SEGMENT)) {
            ...
            if (!checkPayloadLengthAndSetStorage(hexArr, currentSeq)) {
                message.IsFragmentedSegment = true;
            }

        } else {
            message.IsFirstSegment = true;
        ...
        if (message.IsFirstSegment && message.IsFragmentedSegment) {
            message.FragmentedPayload = convertBytesToHex(hexArr.slice(3 + getSourceAddressLength(message.SourceAddress), hexArr.length));
        }
        ...
        if (message.IsFirstSegment && !message.IsFragmentedSegment) {
            ...
            if (!checkFrameLength()) {
                throw "invalid payload: frame length error";
            }
            parsePayLoad(arrayIndex);
        }
        return message;
    } catch (ex) {
        throw new Error("parser failed. " + ex);
    }
}
...
function decodeUplink(input) {
    ...
    if (input.bytes) {
        ...
        return adventechCode(payloadHex);
    } else {
        throw new Error("invalid uplink: no data received");
    }
...
function decodeDownlink(input) {
    ...
    if (input.bytes == null) {
        throw new Error("invalid downlink: no data received");
    }
    ...
    throw new Error("decode downlink is not supported");
}
...
function encodeDownlink(input) {
    ...
    throw new Error("encode downlink is not supported");
