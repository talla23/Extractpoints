...
let payloadHex;
  ...
  switch (result.sensorID) {
    ...
    case 0x1c: //0x1C——R718E
      ...
      break;
    case 0x20: //0x20——R718IA
      result.batteryVoltage = parseFloat((hexToDec(payloadHex, 6, 8) * 0.1).toFixed(1));
      result.adcRawValue = hexToDec(payloadHex, 8, 12);
      break; 
    case 0x2A: //0x20——R718IB
      result.batteryVoltage = parseFloat((hexToDec(payloadHex, 6, 8) * 0.1).toFixed(1));
      result.adcRawValue = hexToDec(payloadHex, 8, 12);
      break;
    case 0x22: //0x22——R718KA
    ...
    case 0x26://0x26——R718MA
      result.batteryVoltage = hexToDec(payloadHex, 6, 8) * 0.1;
      result.RSSI = hexToDec(payloadHex, 8, 12);
      result.SNR = hexToDec(payloadHex, 12, 14);
      result.HeartInterval = hexToDec(payloadHex, 14, 18);
      break;
    case 0x4A: //0x4A——R718N325
      result.batteryVoltage = parseInt((hexToDec(payloadHex, 6, 8) & 0x7F)) * 0.1;
      if(result.reportType == 0x01){
        result.current1 = hexToDec(payloadHex, 8, 12);
        result.current2 = hexToDec(payloadHex, 12, 16);
        result.current3 = hexToDec(payloadHex, 16, 20);
        result.multiplier1 = hexToDec(payloadHex, 20, 22);
      } else if(result.reportType == 0x02){
        result.multiplier2 = hexToDec(payloadHex, 8, 10);
        result.multiplier3 = hexToDec(payloadHex, 10, 12);
      }
      break;
    default:
      result.decoderAvailable = false;
